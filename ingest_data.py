...
# --- START: NEW PRE-PROCESSING FUNCTIONS ---
def normalize_text(text):
    """Removes extra whitespace and line breaks."""
    return re.sub(r'\s+', ' ', text).strip()

def standardize_terms(text):
    """Replaces synonyms with a canonical term."""
    term_map = {
        r'\bplay park\b': 'IPIC Play',
        r'\bplay area\b': 'IPIC Play',
        r'\bgym\b': 'IPIC Active',
        # Add more case-insensitive replacements here as needed
    }
    for old, new in term_map.items():
        text = re.sub(old, new, text, flags=re.IGNORECASE)
    return text
# --- END: NEW PRE-PROCESSING FUNCTIONS ---

def unified_ingestion_pipeline():
    """
    A unified pipeline that pre-processes data, generates a knowledge graph,
    and creates an enriched vector store.
    """
    ...
    print("Step 4: Pre-processing documents...")
    preprocessed_docs = []
    for doc in all_docs:
        cleaned_content = standardize_terms(doc.page_content)
        cleaned_content = normalize_text(cleaned_content)
        preprocessed_docs.append(Document(page_content=cleaned_content, metadata=doc.metadata))
            
    # --- 5. Chunk Documents ---
    print("Step 5: Chunking pre-processed documents...")
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=100)
    chunks = text_splitter.split_documents(preprocessed_docs)
    print(f"Created {len(chunks)} document chunks.")

    # --- 6. Generate Graph, Enrich Metadata, and Add Node Embeddings ---
    print("Step 6: Generating graph, enriching metadata, and adding node embeddings...")
    ...
    # --- 7. Populate the Knowledge Stores ---
    print("Step 7: Populating Neo4j and Supabase...")
